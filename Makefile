...
ifeq ($(MK_PLATFORM_GENERATED),y)
buildall: $(OUTPUT_TARGET_BIN) $(OUTPUT_TARGET_HEX)
else
buildall: buildallwithautogen
endif
...
#autogenerate Kconfig & makefile
.PHONY: autogen 
autogen: 
	#platform Kconfig
	@mkdir -p $(dir $(OUTPUT_PATH)/Kconfig/platform.autogen)
	@echo "#Autogenerated file. Do not edit" 				 > $(OUTPUT_PATH)/Kconfig/platform.autogen
	@echo -n "#" 																		>> $(OUTPUT_PATH)/Kconfig/platform.autogen
	@date 																					>> $(OUTPUT_PATH)/Kconfig/platform.autogen
	@for platform in $(shell ls -d $(AUSBEE_DIR)/Platforms/*/); do \
		cat $$platform/Kconfig 												>> $(OUTPUT_PATH)/Kconfig/platform.autogen; \
	done
	#platform mk
	@mkdir -p $(dir $(OUTPUT_PATH)/mk/platform.autogen)
	@echo "#Autogenerated file. Do not edit" 				 > $(OUTPUT_PATH)/mk/platform.autogen
	@echo -n "#" 																		>> $(OUTPUT_PATH)/mk/platform.autogen
	@date 																					>> $(OUTPUT_PATH)/mk/platform.autogen
	@echo "MK_PLATFORM_GENERATED=y" 				 				>> $(OUTPUT_PATH)/mk/platform.autogen
	@for platform in $(shell ls -d $(AUSBEE_DIR)/Platforms/*/); do \
		PTMP=$$(echo $$platform | sed "s/.*Platforms\/\(.*\)\//\\1/g");\
		UPTMP=$$(echo $$PTMP | sed "s/\./_/g" | sed "s/./\\U\0/g"); \
		echo -n 'ifeq ($$(CONFIG_PLATFORM_' 					>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo -n $$UPTMP 															>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo '),y)' 																	>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo -n 'PLATFORM_INCLUDES+=-I"' 							>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo -n $(AUSBEE_DIR)/Platforms/$$PTMP 				>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo '"' 																			>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo -n 'PLATFORM_SRC_FILES+=$$(wildcard ' 			>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo -n $(AUSBEE_DIR)/Platforms/$$PTMP/ 			>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo '*.c)'																		>> $(OUTPUT_PATH)/mk/platform.autogen; \
		echo 'endif' 																	>> $(OUTPUT_PATH)/mk/platform.autogen; \
	done

#regenerate file then run all
buildallwithautogen:
	$(MAKE) autogen
	$(MAKE) all
...
.PHONY: %-defconfig
%-defconfig: $(CONF) $(KCONFIG_AUTOGEN_LIST) 
...
.PHONY: alldefconfig
alldefconfig: $(CONF) $(KCONFIG_AUTOGEN_LIST)  
...
.PHONY: config
config: $(CONF) $(KCONFIG_AUTOGEN_LIST) 
...
.PHONY: menuconfig
menuconfig: $(MCONF) $(KCONFIG_AUTOGEN_LIST)
...
.PHONY: nconfig
nconfig: $(NCONF) $(KCONFIG_AUTOGEN_LIST) 
...
.PHONY: xconfig
xconfig: $(QCONF) $(KCONFIG_AUTOGEN_LIST) 
...
.PHONY: gconfig
gconfig: $(GCONF) $(KCONFIG_AUTOGEN_LIST) 
